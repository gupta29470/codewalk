...
import { useTheme, PRESETS } from "@/lib/kg/themes";
import type { HeadingFont } from "@/lib/kg/themes";
...
export function ThemePicker() {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 px-2 py-1 rounded text-xs text-kg-text-secondary hover:text-kg-text-primary transition-colors"
        title="Theme"
      >
        <svg
          width="14"
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-64 kg-glass-heavy rounded-lg shadow-2xl z-50 p-3 space-y-3 animate-kg-fade-slide-in">
          {/* Presets */}
          <div>
            <div className="text-[10px] font-semibold text-kg-text-muted uppercase tracking-wider mb-2">
              Theme
            </div>
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded text-xs transition-colors ${
                    p.id === config.presetId
                      ...
                      : "text-kg-text-secondary hover:text-kg-text-primary hover:bg-kg-elevated"
                  }`}
                >
                  <div className="flex gap-1">
                    <span
                      className="w-3 h-3 rounded-full border border-kg-border-subtle"
                  ...
                  {p.id === config.presetId && (
                    <svg
                      className="ml-auto w-3.5 h-3.5 text-kg-accent"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="3"
                ...
                <button
                  ...
                  className={`w-6 h-6 rounded-full transition-transform hover:scale-110 ${
                    swatch.id === config.accentId
                      ? "ring-2 ring-kg-text-primary ring-offset-1 ring-offset-kg-root"
                      : ""
                  }`}
